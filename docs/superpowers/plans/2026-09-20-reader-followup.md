...
- [ ] Diagnose the frozen selector on fresh train/validation tasks: full key,
  operation authority, precedence, false abstention, 4/8 slots and current counts.
- [ ] Implement a small inspectable reader ablation with meaningful tests. Use
...
- [ ] Freeze the final architecture, training configuration and new evaluation
...
- [ ] Test three training seeds, current-record/occupancy changes and longer
  evidence; pair every reading condition with the same reader without memory.
- [ ] Independently replicate uniform A with frozen retention checkpoints and
...
- [ ] Design Case 5B-v2 using observed past access and lifecycle events, budgeted
